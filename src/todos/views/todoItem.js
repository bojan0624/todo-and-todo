...
const TodoItem = ({onToggle, id, onRemove, onAddTag, completed, text, typeId, tagTypes, getColorByType}) => {
  return (
    ...
    >
      ...
      <div style={{margin: '0 10px', textDecoration: completed? 'line-through': 'none'}}>{text}</div>
      {typeId && <Tag style={{width: '20px', height: '20px'}} color={getColorByType(tagTypes, typeId)}/>}
    </List.Item>
  ) 
}
